...
class _AdvancedFeaturesScreenState extends State<AdvancedFeaturesScreen> {
  ...
  Future<void> _demoTransactions() async {
    ...
    try {
      ...
      final product = await _productRepository.insert(
        Product(
          name: 'Test Product',
          price: 99.99,
  ...
  Future<void> _demoComplexQueries() async {
    ...
    try {
      ...
      final result = await NativeSqlite.query('example_app', '''
        SELECT
          o.id as order_id,
          o.quantity,
          o.totalPrice,
      ...
      final statsResult = await NativeSqlite.query('example_app', '''
        SELECT
          status,
          COUNT(*) as count,
      ...
      for (var stat in statsResult.toTypedList()) {
        _appendOutput(
          '  ${stat.getString('status')}: ${stat.getInt('count')} orders, '
  ...
  Future<void> _demoBatchOperations() async {
    ...
    try {
      ...
      if (success) {
        ...
        _appendOutput(
          'Average: ${stopwatch.elapsedMilliseconds / 50}ms per insert',
        );
