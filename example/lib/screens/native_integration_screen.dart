...
class NativeIntegrationScreen extends StatefulWidget {
  ...
  @override
  State<NativeIntegrationScreen> createState() => _NativeIntegrationScreenState();
}

class _NativeIntegrationScreenState extends State<NativeIntegrationScreen> {
  static const platform = MethodChannel('com.example.native_sqlite_example/native');
  ...
  Future<void> _getUsersFromNative() async {
    setState(() => _isLoading = true);
    try {
      final List<dynamic> users = await platform.invokeMethod('getUsersFromNative');

      final buffer = StringBuffer();
      buffer.writeln('Users fetched from native ${Platform.isAndroid ? 'Kotlin' : 'Swift'} code:\n');
  ...
  Widget build(BuildContext context) {
    final platformName = Platform.isAndroid ? 'Android (Kotlin)' : 'iOS (Swift)';
